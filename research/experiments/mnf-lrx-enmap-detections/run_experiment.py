"""
mnf-lrx-enmap-detections: MNF Compression + Local RX anomaly detection on EnMAP.

Per scene:
  1. MNFCompressionLRXDetector (3 components) → LRX anomaly scores
...
logger = logging.getLogger("mnf-lrx-enmap-detections")
...
def save_detection_overlay(
    ...
    """
    Two-panel figure:
      Left:  MNF-LRX score map (inferno)
    ...
    computed = spatial_mask & ~np.isnan(display_map)
    valid_scores = display_map[computed]
    ...
    ax_scores.set_title("MNF-LRX Anomaly Scores", fontsize=11)
    ax_scores.axis("off")
    ...
    fig.suptitle(
        f"{scene_id[:60]}\n"
        f"{n_valid_pixels:,} valid px | {n_scored_pixels:,} scored | "
        f"{n_good_bands} bands → {n_components} MNF components | "
        f"outer={OUTER_WINDOW} inner={INNER_WINDOW} | "
...
def run_single(enmap_folder: Path) -> dict:
    ...
    # 2. Run MNF-LRX detection
    t1 = time.time()
    detector = MNFCompressionLRXDetector(vendable)
    detector.fit(
    ...
    computed_mask = result.computed_mask

    # 3. Threshold at p99.5
    valid_scores = score_map[computed_mask]
    threshold = np.percentile(valid_scores, DETECTION_PERCENTILE)
    det_mask = (
        (score_map >= threshold) & computed_mask
    ).astype(np.uint8)
    ...
    # 4. Write GeoTIFF
    tif_path = scene_dir / "anomaly_mnf_lrx.tif"
    ...
    save_detection_overlay(
        ...
        result.n_good_bands, result.n_valid_pixels, result.n_scored_pixels,
    )
    ...
    return {
        ...
        "tif_name": f"{scene_id}/anomaly_mnf_lrx.tif",
    }
...
def write_summary(results: list[dict]) -> None:
    lines = [
        ...
        f"- **LRX outer window:** {OUTER_WINDOW}",
        f"- **LRX inner window:** {INNER_WINDOW}",
        f"- **Stride:** {STRIDE}",
    ...
    for r in results:
        ...
        lines.append(f"- Scored pixels: {r['n_scored_pixels']:,}")
