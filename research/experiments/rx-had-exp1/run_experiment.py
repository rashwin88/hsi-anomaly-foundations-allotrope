"""
rx-had-exp1: Run GlobalRXDetector on all PRISMA HE5 files in test_payloads.

Deduplicates by filename, builds vendables via PrismaDatasetBuilder,
runs detection, saves visualizations, and writes a summary markdown.
"""
...
logger = logging.getLogger("rx-had-exp1")
...
def run_single(he5_path: Path) -> dict:
    """Build vendable, run RX, save visualization. Returns summary dict."""
...
def write_summary(results: list[dict]) -> None:
    ...
    for r in results:
        ...
        lines.append(
            f"| {r['scene_id'][:40]} | {shape_str} | {bands_str} | {valid_str} | "
            f"{r['pixel_band_ratio']} | {score_range} | {r['score_median']} | "
